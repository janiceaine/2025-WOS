...
public class AccountController : Controller
{
    ...
    public async Task<IActionResult> Profile()
    {
        ...
        if (user == null)
        {
            return RedirectToAction("LoginForm", "Account", new { message = "user-not-found" });
        }

        var vm = new UserJokeStatsViewModel
        {
            UserId = user.Id,
            UserName = user.UserName,
            Email = user.Email,
            JokesCount = _context.Jokes.Count(j => j.UserId == user.Id),
            JokesRated = _context.Ratings.Count(r => r.UserId == user.Id),
        };
